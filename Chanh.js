    ...
    class AutomationManager {
        ...
        */
        async scheduleTask(taskName, taskAction, interval) {
            ...
            // Chỉ cập nhật trạng thái nút, không reload toàn bộ profile.
            if (isTaskDone) {
                updateAllQuestButtons().catch(() => { });
                return;
            }
            ...
            if (nextTime === null || now >= nextTime) {
                ...
                const humanDelay = taskName === 'luyenDan' ? (500 + Math.floor(Math.random() * 1500)) : (2000 + Math.floor(Math.random() * 1700));
                console.log(`[Auto] Đã đến giờ làm nhiệm vụ: ${taskName}. Chờ trước khi thực hiện: ${humanDelay}ms...`);
                ...
                try {
                    ...
                    // Chỉ cập nhật trạng thái nút sau khi task chạy xong.
                    // Không reload toàn bộ profile để tránh thêm các request GET không cần thiết.
                    updateAllQuestButtons().catch(() => { });
